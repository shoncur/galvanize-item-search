...
from PyQt5.QtWidgets import QProgressDialog, QLineEdit, QApplication, QWidget, QGroupBox, QVBoxLayout, QHBoxLayout, QGridLayout, QPushButton, QLabel, QFileDialog, QListWidget, QRadioButton, QMessageBox, QDialog
...
class PDFReader(QWidget):
    ...
    def init_ui(self):
        ...
        # Create widgets
        self.browse_button = QPushButton('Choose a File...', self)
        self.browse_button.clicked.connect(self.browse_pdf)
        self.browse_label = QLabel('No PDF file selected', self)
        self.browse_label.setAlignment(Qt.AlignmentFlag.AlignCenter)

        self.unique_label = QLabel('Unique Item Numbers:', self)
        self.list_widget_unique = QListWidget(self)
        self.duplicate_label = QLabel('Duplicate Item Numbers:', self)
        self.list_widget_duplicates = QListWidget(self)
        ...
        self.download_button = QPushButton('Download Results', self)
        ...
        layout.addWidget(self.supported_numbers_button, 0, 1)
        layout.addWidget(self.browse_label, 1, 0, 1, 2)
        # left_layout.addWidget(self.page_number_label)
        # left_layout.addWidget(self.show_page_numbers_radio)
        

        # GroupBox for the item numbers
        group_box = QGroupBox()
        group_layout = QGridLayout()
        group_layout.addWidget(self.unique_label, 0, 0, Qt.AlignmentFlag.AlignTop)
        group_layout.addWidget(self.duplicate_label, 0, 1, Qt.AlignmentFlag.AlignTop)
        group_layout.addWidget(self.list_widget_unique, 1, 0)
        group_layout.addWidget(self.list_widget_duplicates, 1, 1)
        group_box.setLayout(group_layout)

        # Change background colour for GroupBox
        group_box.setStyleSheet("QGroupBox { background-color: #e3e3e3; border: 1px solid #aaaaaa; padding-top: 2px; }")
    ...
    def browse_pdf(self):
        ...
        if file_name:
            self.browse_label.setText(f'Selected file: {file_name}')
            self.extract_numbers(file_name)
        else:
            self.browse_label.setText('No PDF file selected')
            self.list_widget_unique.clear()
            self.list_widget_duplicates.clear()
            self.download_button.setEnabled(False)
    ...
    def extract_numbers(self, file_name):
        ...
        if matches:
            for match, pattern, page_num in matches:
                ...
                self.list_widget_duplicates.addItem(item)
                print(f"'{match}'")
            ...
            for index, match in enumerate(unique_matches):
                ...
                try: # Get the item from the items world in Arena
                    ...
                    self.list_widget_unique.addItem(f'{match} || {item_lifecycle_phase} | Rev {item_revision}')
                    print(f"'{match}'")
                except Exception as error: 
                    try: # Try to get the item from the supplieritems world
                        ...
                        try:
                            ...
                            self.list_widget_unique.addItem(f'{match} | {linked_item} | {linked_item_lifecycle_phase} | Rev {linked_item_revision}')
                        except:
                            self.list_widget_unique.addItem(f'{match} | Something went wrong, please check item # in Arena to verify the issue')
                            print(f'{error}')
                    except:
                        self.list_widget_unique.addItem(f'{match} | Item not found in Arena')
                        print(f'{error}')
            ...
            self.list_widget_unique.addItem('Nothing found')
            self.download_button.setEnabled(False)
    ...
    def toggle_page_numbers(self):
        if self.browse_label.text() != 'No PDF file selected':
            self.extract_numbers(self.browse_label.text().split(': ')[1])  # Re-extract and display matches based on new mode
        # else:
    ...
    def download_matches(self):
        if self.list_widget_unique.count() == 0:
        ...
        if file_name:
            if file_name.lower().endswith('.csv'):
                with open(file_name, 'w', newline='') as file:
                    ...
                    unique_header = ['Part Number', 'Galvanize Equivalent', 'Lifecycle Status', 'Revision']
                    writer.writerow(unique_header)
                    # Write unique matches without page numbers
                    unique_matches = list(OrderedDict.fromkeys([self.list_widget_unique.item(i).text() for i in range(self.list_widget_unique.count())]))
                    for item in unique_matches:
                        writer.writerow(item.split('|'))  # Split into cells if "|" is present

                    separate_unique_duplicates = ['----------']
                    writer.writerow(separate_unique_duplicates)

                    duplicate_header = ['Page Number', 'Part Number']
                    writer.writerow(duplicate_header)
                    # Write all matches with page numbers
                    for i in range(self.list_widget_duplicates.count()):
                        item = self.list_widget_duplicates.item(i).text()
